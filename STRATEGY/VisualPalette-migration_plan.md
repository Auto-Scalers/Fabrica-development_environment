...
> "**Style:** blueprint/draftsman theme — cream `#f5f2eb`, charcoal `#1a1a1a`, amber `#f2a93b`, teal `#1a8c7b`, neo-brutalist thick borders, drafting-grid background, monospace technical labels, + a dark variant (theme toggle). Source palette extracted from the brand slides via NotebookLM."
...
- Fluid clamp-based scale everywhere (never clips, fits all screens)
- Zero decorative blur; shadow turned off by default (flat engineering style), shadows appear only on interaction/floating layers

---

## 2. Brand Logo & Colors

- **Logo icon:** `Fabrica-ADE/fabrica-logo_icon.ico` (SVG source at `STRATEGY/fabrica-logo_icon.svg`, PNG at `..._icon.png`, plus `fabrica-logo.jpg`). A mark drawn like a drafting/technical emblem.
...
| `*Copper accent` | `#CC7A4A` (inline) | Landing page copper hover (`Fabrica` wordmark dot) |
...
4. **Fluid scaling:** every size is `clamp()` — never fixed px for text/padding/gap on the dashboard panes.
5. **Light & dark both supported** via `data-theme="dark"`. Dark never uses pure black — `#121214` bg, `#2C2C2E` borders so panels stay distinguishable.
6. **Both codebases must converge on this:** the old-fabrica palette (this doc) is the design north star; the Orca-based Fabrica UI must adopt these tokens instead of inventing new colors.
